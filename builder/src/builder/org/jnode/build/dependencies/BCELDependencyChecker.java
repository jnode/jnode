...
public class BCELDependencyChecker extends AbstractPluginTask {    
            ...
            if(plugin != null) {
                ...
                while(iter.hasNext()) {
                    ...
                    if(!containedClasses.containsKey(containedClass)) {
                        ...
                        System.err.println(((Plugin)containedClasses.get(containedClass)).getFullPluginId()+" and "+plugin.getFullPluginId());
    ...
    private class Plugin {
        ...
        private boolean assortClassesContainedInImportedPlugins(StringBuffer buffer, Plugin plugin, 
            ...
            for(int i=0;i<prerequisites.length;i++) {
                String idOfUsedPlugin = createFullPluginId(prerequisites[i].getPluginId(), prerequisites[i].getPluginVersion());
        ...
        private void analyzeConstantPool(String usingClass, JavaClass javaClass) {
            ...
            for(int i=0;i<constants.length;i++) {
                if(constants[i] instanceof ConstantClass) {
                    ...
                    Iterator iter = decodeSignature(((ConstantNameAndType)constants[i]).getSignature(constantPool)).iterator();
                    while(iter.hasNext()) {
    ...
    private class Fragment extends Plugin {
        ...
        private Fragment(JarFile jarFile, FragmentDescriptor descr) {
            super(jarFile, descr);
            this.fullPluginIdOfOwningPlugin = createFullPluginId(((FragmentDescriptor)descr).getPluginId(), ((FragmentDescriptor)descr).getPluginVersion());
        }
        ...
        public String toString() {
            return "Fragment "+fullPluginId+" owned by="+fullPluginIdOfOwningPlugin+" contained="+containedClasses+" used="+usedClasses;
        }
    }
