...
/**
 * We had many cahnges in File.java, URLStreamHandler.java etc. to handle path
 * representations on different platforms (Windows/Unix-family). Finally we'd
 * like to collect all these ad hoc codes into this utility class. --Gansha
 */
...
public class PlatformHelper {
	...
	/**
	 * On most platforms 260 is equal or greater than a max path value, so we
	 * can set the initial buffer size of StringBuffer to half of this value to
	 * improve performance.
	 */
	 ...
	 * path prefix. if not, return 0; if yes, return the len of root path
	 * prefix; --for Unix-family platform, root path begins with "/" and len is
	 * 1 --for Windows platform, root path begins with "drive:\\" and len is 3
	 */
	 ...
	 * --for Unix-family platform, root directory is "/" --for Windows platform,
	 * root directory is "\\" or "drive:\\".
	 */
	public static final boolean isRootDirectory(String path) {
		...
		if (!isWindows)
			return false;
		if (path.length() > 2 && path.length() <= 3 && Character.isLetter(path.charAt(0)))
			return true;
		return false;
	}
	 ...
	 * representation for current platform, including interpreting ".." and "." .
	 */
	public static final String toCanonicalForm(String path) {
		...
		/*
		 * The original "canonpath = new StringBuffer(tmppath.substring(0, i))"
		 * isn't very efficient because StringBuffer's
		 * ensureCapacity_unsynchronized will fail definitely each time and will
		 * enlarge buffer and copy contents. .
		 */
	...
	/**
	 * This routine canonicalizes input param "path" to formal path
	 * representation for current platform, and normalize all separators to
	 * "sepchar".
	 */
	...
	/**
	 * This routine removes from input param "path" the tail separator if it
	 * exists, and return the remain part.
	 */
	...
	/**
	 * This routine returns last index of separator in input param "path", and
	 * return it.
	 */
