...
public class LineBreakIterator extends BaseBreakIterator {
	...
	public int next() {
		...
		while (iter.getIndex() < end) {
			...
			if (n == CharacterIterator.DONE || type == Character.PARAGRAPH_SEPARATOR || type == Character.LINE_SEPARATOR)
				break;

			// Handle two cases where we must scan for non-spacing marks.
			int start = iter.getIndex();
			if (type == Character.SPACE_SEPARATOR || type == Character.START_PUNCTUATION || isIdeo(c)) {
				...
				if (type == Character.SPACE_SEPARATOR) {
					...
					if (nt != Character.NON_SPACING_MARK && nt != Character.SPACE_SEPARATOR && !isNb(n))
						break;
	...
	public int previous() {
		...
		while (iter.getIndex() >= start) {
			...
			if (nt == Character.PARAGRAPH_SEPARATOR || nt == Character.LINE_SEPARATOR)
				break;
			...
			if (nt == Character.SPACE_SEPARATOR && type != Character.SPACE_SEPARATOR && type != Character.NON_SPACING_MARK && !isNb(c))
				break;
