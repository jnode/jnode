...
public class SentenceBreakIterator extends BaseBreakIterator {
	...
	public int next() {
		...
		while (iter.getIndex() < end) {
			...
			if (c == '!' || c == '?') {
				...
				while (n != CharacterIterator.DONE && Character.getType(n) == Character.END_PUNCTUATION)
					n = iter.next();
			...
			if (c == '.') {
				...
				if (spcount > 0) {
					...
					while (n != CharacterIterator.DONE && Character.getType(n) == Character.START_PUNCTUATION)
						n = iter.next();
					// Next character must not be lower case.
					if (n == CharacterIterator.DONE || !Character.isLowerCase(n)) {
						iter.setIndex(save2);
						break;
					}
				}
	...
	private final int previous_internal() {
		...
		while (iter.getIndex() >= start) {
			...
			if (!Character.isLowerCase(c) && (nt == Character.START_PUNCTUATION || Character.isWhitespace(n))) {
				int save = iter.getIndex();
				int save_nt = nt;
				char save_n = n;
			...
			if (nt == Character.PARAGRAPH_SEPARATOR) {
				...
				period = iter.getIndex();
				break;
			} else if (Character.isWhitespace(n) || nt == Character.END_PUNCTUATION) {
				int save = iter.getIndex();
				// Skip (java) space, line and paragraph separators.
				while (n != CharacterIterator.DONE && Character.isWhitespace(n))
					n = iter.previous();
