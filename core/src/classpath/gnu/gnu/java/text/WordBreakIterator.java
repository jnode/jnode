...
public class WordBreakIterator extends BaseBreakIterator {
	...
	public int next() {
		...
		while (iter.getIndex() < end) {
			...
			if (type == Character.PARAGRAPH_SEPARATOR || type == Character.LINE_SEPARATOR)
				break;
			...
			if (c != '\'' && !is_letter && type != Character.NON_SPACING_MARK && Character.isLetter(n))
				break;
			...
			if (!is_hira && !is_kata && !is_han && type != Character.NON_SPACING_MARK && (isHira(n) || isKata(n) || isHan(n)))
				break;
			...
			if (is_hira || is_kata || is_han || is_letter) {
				...
				while (n != CharacterIterator.DONE && Character.getType(n) == Character.NON_SPACING_MARK)
					n = iter.next();
				if (n == CharacterIterator.DONE)
					break;
				if ((is_hira && !isHira(n))
					|| (is_kata && !isHira(n) && !isKata(n))
					|| (is_han && !isHira(n) && !isHan(n)) // FIXME: we treat apostrophe as part of a word.  This
				// is an English-ism.
	...
	public int previous() {
		...
		while (iter.getIndex() >= start) {
			...
			// FIXME: we treat apostrophe as part of a word.  This
			// is an English-ism.
			if (n != '\'' && !Character.isLetter(n) && type != Character.NON_SPACING_MARK && is_letter)
				break;
			...
			// Special case Japanese.
			if ((is_hira || is_kata || is_han) && !isHira(n) && !isKata(n) && !isHan(n) && type != Character.NON_SPACING_MARK)
				break;
			...
			if (!is_hira || (!is_letter && c != '\'')) {
				...
				if ((isHira(n) && !is_hira)
					...
					|| (isHan(n) && !is_hira && !is_han) // FIXME: we treat apostrophe as part of a word.  This
				// is an English-ism.
