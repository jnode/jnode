 ...
 */
public class BufferedImage extends java.awt.Image implements RenderedImage
//implements java.awt.image.WritableRenderedImage
{
	...
	final static int[] masks_int = { 0x00ff0000, 0x0000ff00, 0x000000ff, DataBuffer.TYPE_INT };
	final static int[] masks_565 = { 0xf800, 0x07e0, 0x001f, DataBuffer.TYPE_USHORT };
	final static int[] masks_555 = { 0x7c00, 0x03e0, 0x001f, DataBuffer.TYPE_USHORT };
	...
	public BufferedImage(int w, int h, int type) {
		...
		switch (type) {
			...
			case TYPE_USHORT_555_RGB :
				...
				switch (type) {
					...
					cm = new DirectColorModel(cs, 32, // 32 bits in an int
		masks[0], // r
		masks[1], // g
		masks[2], // b
		alpha ? 0xff000000 : 0, premultiplied, masks[3] // data type
	);
				break;
			...
			case TYPE_USHORT_GRAY :
				...
				cm = new ComponentColorModel(cs, bits, alpha, premultiplied, alpha ? Transparency.TRANSLUCENT : Transparency.OPAQUE, dataType);
				break;
		...
		init(cm, cm.createCompatibleWritableRaster(w, h), premultiplied, null, // no properties
		type);
	}
	...
	public BufferedImage(int w, int h, int type, IndexColorModel indexcolormodel) {
		...
		init(indexcolormodel, indexcolormodel.createCompatibleWritableRaster(w, h), false, // not premultiplied (guess)
		null, // no properties
		type);
	}
	...
	public WritableRaster copyData(WritableRaster dest) {
		...
		// create a src child that has the right bounds...
			WritableRaster src = raster.createWritableChild(x, y, w, h, x, y, null // same bands
	);
	...
	public Graphics2D createGraphics() {
		return VMImageUtils.getAPI().createGraphics(this);
	}
	...
	public Raster getData(Rectangle rectangle) {
		WritableRaster dest = raster.createCompatibleWritableRaster(rectangle);
	...
	public int getRGB(int x, int y) {
			Object rgbElem = raster.getDataElements(x, y, null // create as needed
	);
	...
	public ImageProducer getSource() {
		// TODO This should be faster using direct access to raster.getDataBuffer.
		final int w = raster.getWidth();
		final int h = raster.getHeight();
		final int[] pixels = raster.getPixels(0, 0, w, h, (int[])null);
		return new MemoryImageSource(w, h, getColorModel(), pixels, 0, w);
	}
	...
	public BufferedImage getSubimage(int x, int y, int w, int h) {
		WritableRaster subRaster = getRaster().createWritableChild(x, y, w, h, 0, 0, null);

		return new BufferedImage(getColorModel(), subRaster, isPremultiplied, properties);
	}
	...
	public void setData(Raster src) {
		...
		// create a dest child that has the right bounds...
			WritableRaster dest = raster.createWritableChild(x, y, w, h, x, y, null // same bands
	);
	...
	/**
	 * Set a pixel with the given RGB color value at the given x,y location.
	 * @param x
	 * @param y
	 * @param argb
	 */
