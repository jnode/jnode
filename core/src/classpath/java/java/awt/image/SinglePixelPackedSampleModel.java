...
public class SinglePixelPackedSampleModel extends SampleModel {
	...
	public Object getDataElements(int x, int y, int w, int h, Object obj, DataBuffer data) {
		final int size = w * h;
		final int numDataElements = getNumDataElements();
		final int dataSize = numDataElements * size;
		...
		for (int row = 0; row < h; row++) {
			final int offset = (scanlineStride * (y + row)) + x + data.getOffset();
			Buffers.getData(data, offset, obj, row * w, w);
		}
		return obj;
	}
	...
	public int[] getPixels(int x, int y, int w, int h, int[] iArray, DataBuffer data) {
		int offset = scanlineStride * y + x;
		final int iDataSize = numBands * w * h;
		if (iArray == null) {
			iArray = new int[iDataSize];
		} else {
			if (iArray.length < iDataSize) {
				throw new IndexOutOfBoundsException("iArray is too short, it must be " + iDataSize + " long instead of " + iArray.length);
			}
		}
		int outOffset = 0;
		for (y = 0; y < h; y++) {
	...
	public void setDataElements(int x, int y, Object obj, DataBuffer data) {
		...
		if (getTransferType() != data.getDataType()) {
			throw new IllegalArgumentException("transfer type (" + getTransferType() + "), " + "does not match data " + "buffer type (" + data.getDataType() + ").");
		}
	...
	public void setPixel(int x, int y, int[] iArray, DataBuffer data) {
		...
		//System.out.println("Set pixel x=" + x + ", y=" + y + ", ofs=" + offset + ", sample=" + samples);
