...
public class Proxy implements Serializable {
    ...
    public static synchronized boolean isProxyClass(Class clazz) {
        if (!Proxy.class.isAssignableFrom(clazz)) return false;
     ...
     * Configuration.HAVE_NATIVE_GET_PROXY_CLASS is true, this does the work of
     * both getProxyData0 and generateProxyClass0 with no intermediate form in
     * Java. The native code may safely assume that this class must be created,
     ...
     */
    private static ProxyData getProxyData0(ClassLoader loader,
            Class[] interfaces) {
        return null;
    }
    ...
    private static final class ProxyType {
        ...
        private static boolean sameTypes(Class arr1[], Class arr2[]) {
            if (arr1.length == 1 && arr2.length == 1) { return arr1[ 0] == arr2[ 0]; }
        ...
        public boolean equals(Object other) {
            ...
            if (loader != pt.loader
                    || interfaces.length != pt.interfaces.length) return false;
    ...
    private static final class ProxySignature {
        ...
        void checkCompatibility(ProxySignature other) {
            ...
            while (--pos >= 0) {
                ...
                while (--pos2 >= 0) {
                    ...
                    if (c2.isAssignableFrom(c1)) valid1[ pos] = true;
                    if (c1.isAssignableFrom(c2)) valid2[ pos2] = true;
                }
            }
            pos = size1;
            ...
            while (--pos >= 0) {
                itr.next();
                if (!valid1[ pos]) itr.remove();
            }
            pos = size2;
            itr = other.exceptions.iterator();
            while (--pos >= 0) {
                itr.next();
                if (!valid2[ pos]) itr.remove();
            }
        ...
        public boolean equals(Object other) {
            ...
            if (!method.getName().equals(ps.method.getName())
                    || types1.length != types2.length) return false;
            int i = types1.length;
            while (--i >= 0)
            ...
            while (--i >= 0) {
                ...
                while (--j >= 0) {
                    ...
                    ProxySignature old = (ProxySignature) method_set.put(sig,
                            sig);
                    if (old != null) sig.checkCompatibility(old);
                }
            }
    ...
    private static final class ClassFactory {
        ...
        private void emitMethod(int i, Class[] e) {
            ...
            if (paramtypes.length > 0) {
                ...
                if (paramtypes.length > Byte.MAX_VALUE)
                    code_length += 2; // sipush
                else if (paramtypes.length > 5) code_length++; // bipush
                for (int j = 0; j < paramtypes.length; j++) {
                    ...
                    if (param_count >= 4) code_length++; // 2-byte load
                    param_count++;
                    if (type.isPrimitive()) {
                        ...
                        if (type == long.class || type == double.class) {
                            wrap_overhead = 3;
                            param_count++;
                        } else if (wrap_overhead < 2) wrap_overhead = 2;
                    }
                }
            }
        ...
        final Class generate(ClassLoader loader) {
            ...
            try {
                ...
                // XXX Security hole - it is possible for another thread to grab
                // the
                // VMClassLoader.defineClass Method object, and abuse it while
                // we
                // have temporarily made it accessible. Do we need to add some
                // synchronization lock to prevent user reflection while we use
                // it?
                ...
                // is trusted code. But note the comment above about the
                // security
        ...
        private void putLoad(int i, Class type) {
            ...
            if (type == long.class)
                ...
                offset = 3;
            else if (!type.isPrimitive()) offset = 4;
            if (i < 4)
        ...
        private String wrapper(Class clazz) {
            if (clazz == boolean.class) return "java/lang/Boolean";
            if (clazz == byte.class) return "java/lang/Byte";
            if (clazz == short.class) return "java/lang/Short";
            if (clazz == char.class) return "java/lang/Character";
            if (clazz == int.class) return "java/lang/Integer";
            if (clazz == long.class) return "java/lang/Long";
            if (clazz == float.class) return "java/lang/Float";
            if (clazz == double.class) return "java/lang/Double";
            // assert false;
            return null;
        }
