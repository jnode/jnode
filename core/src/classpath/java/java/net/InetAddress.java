...
public class InetAddress implements Serializable {
    ...
    /**
     * Initializes this object's addr instance variable from the passed in int
     * array. Note that this constructor is protected and is called only by
     * static methods in this class.
     * 
     * @param ipaddr
     *            The IP number of this address as an array of bytes
     */
    //private
    ...
    public boolean equals(Object obj) {
        ...
        if (test_ip.length != addr.length) return (false);

        for (int i = 0; i < addr.length; i++)
            if (test_ip[ i] != (byte) addr[ i]) return (false);
    ...
    /**
     * Returns the IP address of this object as a String. The address is in the
     * dotted octet notation, for example, "127.0.0.1".
     * 
    ...
    /**
     * Returns a hash value for this address. Useful for creating hash tables.
    ...
    /**
     * Returns true if this address is a multicast address, false otherwise. An
     * address is multicast if the high four bits are "1110". These are also
     * known as "Class D" addresses.
     * 
    ...
    public boolean isMulticastAddress() {
        ...
        if ((addr[ 0] & 0xF0) == 224) return (true);
    ...
    /**
     * Converts this address to a String. This string contains the IP in dotted
     * decimal form. For example: "127.0.0.1" This method is equivalent to
     * getHostAddress() and overrides Object.toString()
     * 
    ...
    /**
     * This native method looks up the hostname of the local machine we are on.
     * If the actual hostname cannot be determined, then the value "localhost"
     * we be used. This native method wrappers the "gethostname" function.
     * 
    ...
    /**
     * This method returns the hostname for a given IP address. It will throw
     * an UnknownHostException if the hostname cannot be determined.
     * 
     * @param ip
     *            The IP address as a int arrary
     * 
     * @return The hostname
     * 
     * @exception UnknownHostException
     ...
     */
    private static String getHostByAddr(byte[] ip) throws UnknownHostException {
        return null;
    }
     ...
     */
    private static InetAddress[] getHostByName(String hostname)
            throws UnknownHostException {
        ...
        try {
            ...
            if (tok.countTokens() == 4) {
                ...
                for (int i = 0; i < 4; i++) {
                    addr[ i] = (byte) Integer.parseInt(tok.nextToken());
                }
                return new InetAddress[] { InetAddress.getByAddress(addr)};
            }
