 ...
 * access control context of the current thread through the <code>checkPermission()</code>
 * method. Manipulates the access control context for code that needs to be
 * executed the protection domain of the calling class (by explicitly ignoring
 * the context of the calling code) in the <code>doPrivileged()</code>
 * methods. And provides a <code>getContext()</code> method which gives the
 * access control context of the current thread that can be used for checking
 * permissions at a later time and/or in another thread.
 * <p>
 * XXX - Mostly a stub implementation at the moment. Needs native support from
 * the VM to function correctly. XXX - Do not forget to think about how to
 * handle <code>java.lang.reflect.Method.invoke()</code> on the <code>doPrivileged()</code>
 * methods.
 * 
