 ...
 * associated action list. That is, either the named permission is granted or
 * it is not.
 * 
 * <p>
 ...
 * might grant all permissions under the "org.gnu" permissions hierarchy). The
 * only valid wildcard character is a '*' which matches anything. It must be
 * the rightmost element in the permission name and must follow a '.' or else
 * the Permission name must consist of only a '*'. Any other occurrence of a
 * '*' is not valid.
 * 
 * <p>
...
// FIXME extends with fully qualified classname is workaround for gcj 3.0.4.
{
     ...
     * empty, or contains an illegal wildcard character, an exception is
     * thrown.
     * 
     * @param name
    ...
    public BasicPermission(String name) {
        ...
        if ("".equals(name)) throw new IllegalArgumentException("Empty name");
    }
     ...
     * object.</li>
     * <li>The name of the specified permission is implied by this
     * permission's name based on wildcard matching. For example, "a.*" implies
     * "a.b".</li>
     * </ul>
     * 
     * @param perm
    ...
    public boolean implies(Permission perm) {
        if (!getClass().isInstance(perm)) return false;
    ...
    /**
     * This method tests to see if this object is equal to the specified <code>Object</code>.
     * This will be true if and only if the specified object meets the
...
final class BasicPermissionCollection extends PermissionCollection {
    ...
    public boolean implies(Permission permission) {
        if (!permClass.isInstance(permission)) return false;
        ...
        while (true) {
            if (permissions.get(name) != null) return true;
