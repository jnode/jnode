...
/**
 * This class represents a location from which code is loaded (as represented
 * by a URL), and the list of certificates that are used to check the
 * signatures of signed code loaded from this source.
 * 
...
public class CodeSource implements Serializable {
    ...
    /**
     * This is the URL that represents the code base from which code will be
     * loaded.
     * 
     ...
     * from the specified URL location and which uses the specified
     * certificates for verifying signatures.
     * 
     * @param location
     *            the location from which code will be loaded
     * @param certs
     *            the list of certificates
     */
    public CodeSource(URL location, Certificate[] certs) {
        this.location = location;
        if (certs != null) this.certs = new HashSet(Arrays.asList(certs));
    }
     ...
     * 
     * @param obj
     *            the <code>Object</code> to test against
    ...
    public boolean equals(Object obj) {
        ...
        return (certs == null ? cs.certs == null : certs.equals(cs.certs))
                && (location == null ? cs.location == null : location
                        .equals(cs.location));
    }

    /**
     * This method returns the URL specifying the location from which code will
     * be loaded under this <code>CodeSource</code>.
     * 
    ...
    /**
     * This method returns the list of digital certificates that can be used to
     * verify the signatures of code loaded under this <code>CodeSource</code>.
     * 
     ...
     * more), and must have a location that is a subset of this object's. In
     * order for this object to imply the specified object, the following must
     * be true:
     * <ol>
     * <li><em>codesource</em> must not be <code>null</code>.</li>
     * <li>If <em>codesource</em> has a certificate list, all of it's
     * certificates must be present in the certificate list of this code
     * source.</li>
     * <li>If this object does not have a <code>null</code> location, then
     * the following addtional tests must be passed.
     * <ol>
     * <li><em>codesource</em> must not have a <code>null</code> location.
     * </li>
     ...
     * location host, as determined by contructing <code>SocketPermission</code>
     * objects from each with no action list and using that classes's <code>implies</code>
     * method</li>
     ...
     * object's location file with the '/' character appended to it.</li>
     * </ul>
     * </li>
     * </ol>
     * </ol>
     ...
     * </pre>
     * 
     * Note that the code source with null location and null certificates
     * implies all other code sources.
     * 
     * @param cs
     *            the <code>CodeSource</code> to test against this object
    ...
    public boolean implies(CodeSource cs) {
        ...
        if (!location.getProtocol().equals(cs.location.getProtocol())) { return false; }

        final int ourPort = location.getPort();
        if ((ourPort != -1) && (ourPort != cs.location.getPort())) { return false; }

        final String ourRef = location.getRef();
        if ((ourRef != null) && !ourRef.equals(cs.location.getRef())) { return false; }

        final String ourHost = location.getHost();
        if (ourHost != null) {
            final String their_host = cs.location.getHost();
            if (their_host == null) { return false; }
            final SocketPermission our_sockperm = new SocketPermission(ourHost,
                    "accept");
            final SocketPermission their_sockperm = new SocketPermission(
                    their_host, "accept");
            if (!our_sockperm.implies(their_sockperm)) { return false; }
        }
        ...
        if (our_file != null) {
            ...
            final String their_file = cs.location.getFile();
            if (their_file == null || !their_file.startsWith(our_file))
                    return false;
        }
        return true;
    }
     ...
     *             if deserialization fails @serialData this reads the
     *             location, then expects an int indicating the number of
     *             certificates. Each certificate is a String type followed by
     *             an int encoding length, then a byte[] encoding
     */
     ...
     *            the output stream
     * @throws IOException
     *             if writing fails @serialData this writes the location, then
     *             writes an int indicating the number of certificates. Each
     *             certificate is a String type followed by an int encoding
     *             length, then a byte[] encoding
     */
