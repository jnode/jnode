...
public final class VmSystemClassLoader extends VmAbstractClassLoader {
	 ...
	 * @param arch
	 */
	public VmSystemClassLoader(URL classesURL, VmArchitecture arch, ObjectResolver resolver) {
	...
	/**
	 * Gets the loaded class with a given name, or null if no such class has been loaded.
	 * 
	 * @param name
	 * @return VmClass
	 */
	 ...
	 * @throws ClassNotFoundException
	 */
	public VmType loadClass(String name, boolean resolve) throws ClassNotFoundException {
	 ...
	 * @throws ClassNotFoundException
	 */
	private VmType loadNormalClass(String name) throws IOException, ClassNotFoundException {
		...
		return ClassDecoder.defineClass(name, image, 0, image.length, rejectNatives, this, selectorMap, statics);
	}
	 ...
	 * @throws ClassNotFoundException
	 */
	private byte[] getClassStream(String clsName) throws MalformedURLException, IOException, ClassNotFoundException {
		final String fName = clsName.replace('.', '/') + ".class";
		final InputStream is = getResourceAsStream(fName);
		if (is == null) {
			throw new ClassNotFoundException("Class resource of " + clsName + " not found.");
		} else {
	 ...
	 * @return An opened inputstream to the resource with the given name, or null if not found.
	 * @throws MalformedURLException
	 * @throws IOException
	 */
	...
	static class ClassInfo {
		...
		public ClassInfo(VmType vmClass) {
			...
			if (name.indexOf('/') >= 0) {
				throw new IllegalArgumentException("vmClass.getName() contains '/'");
			}
		}
		 ...
		 * @throws ClassNotFoundException
		 */
		public final synchronized VmType getVmClass() throws ClassNotFoundException {
			while (vmClass == null) {
				if (error) {
					throw new ClassNotFoundException(name + "; error on other thread that tried to load it");
				}
				try {
					wait();
