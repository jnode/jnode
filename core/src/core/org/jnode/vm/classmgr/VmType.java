...
public abstract class VmType extends VmSystemObject implements Uninterruptible {
	...
	/** The parent of this class. Normally VmClass instance, during loading String instance */
	...
	private VmType(String name, VmNormalClass superClass, String superClassName, AbstractVmClassLoader loader, boolean primitive, int accessFlags, int typeSize) {
		...
		this.modifiers = accessFlags | Modifier.ACC_LOADED;
	 ...
	 * @return VmClass
	 */
	private final VmArrayClass createArrayClass(boolean link, String arrayClassName) {
		final String name;
		if (arrayClassName != null) {
			name = arrayClassName;
		} else {
		...
		final VmArrayClass arrayClass = new VmArrayClass(name, this.getLoader(), false, this, -1);
		if (link) {
			arrayClass.link();
		}
		return arrayClass;
	...
	/**
	 * Gets the primitive array class corresponding to the given (newarray) type.
	 * 
	 * @param type
	 * @return VmClass
	 */
	...
	public final VmNormalClass getSuperClass() {
		if (superClass != null) {
			...
			throw new RuntimeException("getSuperClass called too early in class " + name);
		}
	}
	...
	public final boolean isCompiled() {
		return Modifier.isCompiled(modifiers);
	}
	public final boolean isLoaded() {
		return Modifier.isLoaded(modifiers);
	}
	public final boolean isVerifying() {
		return Modifier.isVerifying(modifiers);
	}
	public final boolean isVerified() {
		return Modifier.isVerified(modifiers);
	}
	public final boolean isPreparing() {
		return Modifier.isPreparing(modifiers);
	}
	public final boolean isPrepared() {
		return Modifier.isPrepared(modifiers);
	}
	public final boolean isInitializing() {
		return Modifier.isInitializing(modifiers);
	}
	 ...
	 * 
	 * @param method
	 * @return The index of the given method within this class or -1 if not found.
	 */
	 ...
	 * @return The method
	 */
	final VmMethod getMethod(String name, String signature, boolean onlyThisClass, boolean searchInterfaces, int hashCode) {
		...
		if (mt != null) {
			...
			for (int i = 0; i < count; i++) {
				...
				/** Use the hascode as first match, to make this as fast as possible */
				...
				if (mtsHashCode == hashCode) {
					...
					/** The methods are sorted on hashcode, so we can stop searching here */
					break;
				}
			}
		}
		...
		if (isAbstract()) {
			final VmMethod method = getSyntheticAbstractMethod(name, signature, hashCode);
			if (method != null) {
				return method;
			}
		}
		...
		if ((superClass != null) && (!onlyThisClass)) {
			final VmMethod method = superClass.getMethod(name, signature, false, false, hashCode);
			if (method != null) {
				return method;
			}
		}
		...
		if (isInterface() || searchInterfaces) {
			...
			if (ait != null) {
				...
				for (int i = 0; i < count; i++) {
					...
					final VmMethod method = intf.getMethod(name, signature, true, false, hashCode);
					if (method != null) {
						return method;
					}
				}
	 ...
	 * @return The method
	 */
	final VmMethod getMethod(String name, VmType[] argTypes, boolean declaredOnly) {
		...
		if (isInterface() && (!declaredOnly)) {
			...
			if (it != null) {
				...
				for (int i = 0; i < count; i++) {
					...
					VmMethod method = intf.getResolvedVmClass().getMethod(name, argTypes, false);
					if (method != null) {
						return method;
					}
				}
	 ...
	 * @return The method
	 */
	private final VmMethod getMethod(String name, String signature, boolean onlyThisClass, boolean searchInterfaces) {
		return getMethod(name, signature, onlyThisClass, searchInterfaces, VmMember.calcHashCode(name, signature));
	}
	...
	public final VmMethod getDeclaredMethod(String name, String signature) {
		return getMethod(name, signature, true, false, VmMember.calcHashCode(name, signature));
	}
	...
	public final VmMethod getMethod(String name, String signature) {
		return getMethod(name, signature, false, true, VmMember.calcHashCode(name, signature));
	}
	...
	public final VmMethod getMethod(VmConstMethodRef methodRef) {
		return getMethod(methodRef.getName(), methodRef.getSignature(), false, true, methodRef.getMemberHashCode());
	}
	...
	private synchronized VmType doPrepare() {
		...
		try {
			...
			if ((superClass == null) && (superClassName != null)) {
				setSuperClass((VmNormalClass) loader.loadClass(superClassName, false));
			}
			...
			modifiers |= Modifier.ACC_DEFINED;
		} catch (ClassNotFoundException ex) {
			modifiers |= Modifier.ACC_INVALID;
			modifiers &= ~Modifier.ACC_PREPARING;
	 ...
	 * 
	 * @param allInterfaces
	 *            All interfaces directly or indirectly implemented by this class
	...
	protected void setInterfaceTable(VmImplementedInterface[] interfaceTable) {
		if (this.interfaceTable == null) {
			...
			throw new IllegalArgumentException("Cannot overwrite interface table");
		}
	}
	...
	/**
	 * Verify this object before it is written into the bootimage by the bootimage builder.
	 * 
	...
	private synchronized void doInitialize() {
		if (!Modifier.isInitialized(modifiers)) {
			...
			if (!Modifier.isInitializing(modifiers)) {
				modifiers |= Modifier.ACC_INITIALIZING;
				/*
	...
	/**
	 * Gets the <clinit>method or null if no <clinit>method was found in this class.
	 * 
	 * @return VmMethod
	 */
