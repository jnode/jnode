...
public abstract class DoubleWordItem extends Item implements
	...
	/**
	 * Create a DoubleWordItem in a local variable.
	 * 
	 * @param jvmType
	 *            LONG, DOUBLE
	 * @param lsb
	 ...
	 * @return
	 */
	static final DoubleWordItem createLocal(int jvmType, int offset) {
		switch (jvmType) {
		case JvmType.LONG:
			return LongItem.createLocal(offset);
		case JvmType.DOUBLE:
			return DoubleItem.createLocal(offset);
		default:
	...
	protected final Item clone(EmitterContext ec) {
		...
		case Kind.LOCAL:
			res = createLocal(getType(), super.getOffsetToFP());
			break;
