 ...
 * Uses a virtual stack to delay item emission, as described in
 * the ORP project
 * 
...
class X86BytecodeVisitor extends InlineBytecodeVisitor implements X86CompilerConstants {
	...
	/*
	 * Virtual Stack: this stack contains values that have been computed
	 * but not emitted yet; emission is delayed to allow for optimizations,
	 * in particular using registers instead of stack operations.
	 * 
	 ...
	 * Each time a local is assigned, the stack is checked for aliases.
	 * For teh same reason, array and field operations are not delayed.
	 */
	...
	private void PrepareForOperation(Item destAndSource, Item source) {
		// WARNING: source was on top of the virtual stack (thus higher than destAndSource)
		// x86 can only deal with one complex argument
		// destAndSource must be a register
		source.loadIf((Item.STACK | Item.FREGISTER));
		destAndSource.load();
	}
	...
	public final void visit_aaload() {
		...
		idx.loadIf(~Item.CONSTANT);
		ref.load();
		Register refReg = ref.getRegister();
		checkBounds(ref, idx);
		if (idx.getKind() == Item.CONSTANT) {
			final int offset = idx.getValue();
			os.writeMOV(INTSIZE, refReg, refReg, offset+VmArray.DATA_OFFSET * 4);
		} else {
			os.writeMOV(INTSIZE, refReg, refReg, idx.getRegister(), 4, VmArray.DATA_OFFSET * 4);
			idx.release();
		}
...
//		os.writePUSH(S0, T0, 4, VmArray.DATA_OFFSET * 4);
	...
	public final void visit_aastore() {
		...
		if (idx.getKind() == Item.CONSTANT) {
			int i = idx.getValue();
			os.writeMOV(INTSIZE, r, i + VmArray.DATA_OFFSET * 4, val.getRegister());
		} else {
			os.writeMOV(INTSIZE, r, idx.getRegister(), 4, VmArray.DATA_OFFSET * 4, val.getRegister());			
		}
		val.release();
		idx.release();
		ref.release();
...
//		helper.writePUSH(T0);
	...
	public final void visit_aload(int index) {
		vstack.pushItem(RefItem.CreateLocal(index));
...
//		helper.writePOP(T0);
	...
	public final void visit_astore(int index) {
		...
		RefItem i = vstack.popRef();
		i.loadIf(~Item.CONSTANT);
		
		if (i.getKind() == Item.CONSTANT) {
			//TODO: check whether constant is different from NULL (loaded with ldc)
...
//		helper.writePOP(T0); // Exception
	...
	public final void visit_baload() {
		...
		if (idx.getKind() == Item.CONSTANT) {
			final int offset = idx.getValue();
			os.writeMOV(BYTESIZE, refReg, refReg, offset+VmArray.DATA_OFFSET * 4);
		} else {
			os.writeMOV(BYTESIZE, refReg, refReg, idx.getRegister(), 1, VmArray.DATA_OFFSET * 4);
			idx.release();
		}
...
//		helper.writePUSH(T0);
	...
	public final void visit_bastore() {
		...
		if (idx.getKind() == Item.CONSTANT) {
			int i = idx.getValue();
			os.writeMOV(BYTESIZE, r, i + VmArray.DATA_OFFSET * 4, val.getRegister());
		} else {
			os.writeMOV(BYTESIZE, r, idx.getRegister(), 4, VmArray.DATA_OFFSET * 4, val.getRegister());			
		}
		val.release();
		idx.release();
		ref.release();
...
//	}
	...
	public final void visit_caload() {
		...
		if (idx.getKind() == Item.CONSTANT) {
			final int offset = idx.getValue();
			os.writeMOV(WORDSIZE, refReg, refReg, offset+VmArray.DATA_OFFSET * 4);
		} else {
			os.writeMOV(WORDSIZE, refReg, refReg, idx.getRegister(), 2, VmArray.DATA_OFFSET * 4);
			idx.release();
		}
...
//		helper.writePUSH(T0);
	...
	public final void visit_castore() {
		...
		if (idx.getKind() == Item.CONSTANT) {
			int i = idx.getValue();
			os.writeMOV(WORDSIZE, r, i + VmArray.DATA_OFFSET * 4, val.getRegister());
		} else {
			os.writeMOV(WORDSIZE, r, idx.getRegister(), 2, VmArray.DATA_OFFSET * 4, val.getRegister());			
		}
		val.release();
		idx.release();
		ref.release();
...
//		visit_lconst(Double.doubleToLongBits(value));
	...
	public final void visit_dload(int index) {
		vstack.pushItem(DoubleItem.CreateLocal(index));
...
//		|os.writeFFREE(Register.ST0);
	...
	public final void visit_faload() {
		...
		if (idx.getKind() == Item.CONSTANT) {
			...
			//os.writeMOV(INTSIZE, refReg, refReg, idx.getRegister(), 2, VmArray.DATA_OFFSET * 4);
...
//		helper.writePUSH(Register.EAX);
	...
	public final void visit_fload(int index) {
		vstack.pushItem(FloatItem.CreateLocal(index));
//		final int ebpOfs = stackFrame.getEbpOffset(index);
//		os.writeMOV(INTSIZE, T0, FP, ebpOfs);
//		helper.writePUSH(T0);
	}
	...
	public final void visit_getfield(VmConstFieldRef fieldRef) {
		//TODO: port to orp-style; must probably implement a getfield method in each Item!
		fieldRef.resolve(loader);
		final VmField field = fieldRef.getResolvedVmField();
		if (field.isStatic()) {
			throw new IncompatibleClassChangeError("getfield called on static field " + fieldRef.getName());
		}
...
//		helper.writePUSH(T0);
	...
	public final void visit_iadd() {
		//REFACTOR: parametrize the os.write operations to avoid code duplication
		...
		switch(v2.getKind()) {
			case Item.REGISTER:
				...
				break;
			case Item.LOCAL:
				os.writeADD(v1.getRegister(), stackFrame.getEbpOffset(v2.getLocal()), FP);
				break;
			case Item.CONSTANT:
				//IMPROVE: implement ADD(reg, imm32)
				// os.writeADD(v1.getRegister(), ((IntConstant)v2.getConstant()).getValue());
				v2.load();
				os.writeADD(v1.getRegister(), v2.getRegister());
				break;
		}
	...
	public final void visit_iand() {
		...
		switch(v2.getKind()) {
			case Item.REGISTER:
				...
				break;
			case Item.LOCAL:
				os.writeAND(v1.getRegister(), stackFrame.getEbpOffset(v2.getLocal()), FP);
				break;
			case Item.CONSTANT:
	...
	public final void visit_iastore() {
		...
		if (idx.getKind() == Item.CONSTANT) {
			...
			if (val.getKind() == Item.CONSTANT) {
				...
				//os.writeMOV_Const(r, idx.getRegister(), 4, VmArray.DATA_OFFSET * 4, vc);
			} else {
				os.writeMOV(INTSIZE, r, idx.getRegister(), 4, VmArray.DATA_OFFSET * 4, val.getRegister());
			}
		}
	...
	private final void visit_if_xcmp(int address, int jccOpcode) {
		...
		switch(v2.getKind()) {
			case Item.REGISTER:
				...
				break;
			case Item.LOCAL:
				os.writeCMP(r1, stackFrame.getEbpOffset(v2.getLocal()), FP);
				break;
			case Item.CONSTANT:
				int c2 = v2.getValue();
				os.writeCMP(r1, c2);
...
//		os.writeJCC(helper.getInstrLabel(address), jccOpcode);
	...
	public final void visit_iinc(int index, int incValue) {
		vstack.loadLocal(index);		// avoid aliasing throubles
	...
	public final void visit_iload(int index) {
		vstack.pushItem(IntItem.CreateLocal(index));
//		final int ebpOfs = stackFrame.getEbpOffset(index);
//		os.writeMOV(INTSIZE, T0, FP, ebpOfs);
//		helper.writePUSH(T0);
	}
...
//		helper.writePUSH(T0);
	...
	public final void visit_invokeinterface(VmConstIMethodRef methodRef, 
		...
		/*
		 * if (!methodRef.getConstClass().isResolved()) { Label startClassLabel = new
		 * Label(this.curInstrLabel + "startClass"); os.setObjectRef(startClassLabel);
		 * resolveClass(methodRef.getConstClass()); patch_NOP(startClassLabel); }
		 * 
		 * if (!methodRef.isResolved()) { Label startLabel = new Label(this.curInstrLabel +
		 * "start"); os.setObjectRef(startLabel); resolveMethod(methodRef); patch_NOP(startLabel);
		 */
		...
		final Label findSelectorLabel = new Label(this.curInstrLabel + "FindSelector");
		...
		/*
		 * // methodRef -> EDX os.writeMOV_Const(Register.EDX, methodRef); // methodRef.selector ->
		 * ecx os.writeMOV(INTSIZE, Register.ECX, Register.EDX,
		 * context.getVmConstIMethodRefSelectorField().getOffset()); // methodRef.selector -> eax
		 * os.writeMOV(INTSIZE, Register.EAX, Register.ECX); // Clear edx os.writeXOR(Register.EDX,
		 * Register.EDX); // IMT_LENGTH -> ESI os.writeMOV_Const(Register.ESI,
		...
		/* objectref.TIB -> ebx */
		os.writeMOV(INTSIZE, Register.EBX, Register.EBX, ObjectLayout.TIB_SLOT * slotSize);
		/* boolean[] imtCollisions -> esi */
		os.writeMOV(INTSIZE, Register.ESI, Register.EBX, (VmArray.DATA_OFFSET + TIBLayout.IMTCOLLISIONS_INDEX) * slotSize);
		/* Has collision at imt[index] ? */
		os.writeMOV(INTSIZE, Register.EAX, Register.ESI, Register.EDX, 1, VmArray.DATA_OFFSET * slotSize);
		os.writeTEST_AL(0xFF);
		/* Object[] imt -> esi */
		os.writeMOV(INTSIZE, Register.ESI, Register.EBX, (VmArray.DATA_OFFSET + TIBLayout.IMT_INDEX) * slotSize);
		/* selector -> ebx */
		...
		os.writeMOV(INTSIZE, Register.ESI, Register.ESI, Register.EDX, 4, VmArray.DATA_OFFSET * slotSize);
		/* collisionList.length -> ecx */
		os.writeMOV(INTSIZE, Register.ECX, Register.ESI, VmArray.LENGTH_OFFSET * slotSize);
		/* &collisionList[0] -> esi */
		...
		os.writeMOV(INTSIZE, Register.EDX, Register.EAX, context.getVmMethodSelectorField().getOffset());
		...
		/* imt[index] -> eax */
		os.writeMOV(INTSIZE, Register.EAX, Register.ESI, Register.EDX, 4, VmArray.DATA_OFFSET * slotSize);
	...
	public final void visit_invokespecial(VmConstMethodRef methodRef) {
		...
		helper.invokeJavaMethod(methodRef.getSignature());
			...
			System.out.println(methodRef.getResolvedVmMethod().getClass().getName() + "#" + methodRef.getName());
			throw ex;
		}
	...
	public final void visit_invokestatic(VmConstMethodRef methodRef) {
		...
		methodRef.resolve(loader);
		final VmStaticMethod sm = 
			(VmStaticMethod)methodRef.getResolvedVmMethod();
	...
	public final void visit_invokevirtual(VmConstMethodRef methodRef) {
		...
		if (mts.isStatic()) {
			throw new IncompatibleClassChangeError("Static method in invokevirtual");
		}
		final VmInstanceMethod method = (VmInstanceMethod) mts;
		final int tibOffset = method.getTibOffset();
		final int argSlotCount = 
			Signature.getArgSlotCount(methodRef.getSignature());
		...
		os.writeMOV(INTSIZE, EAX, S0, (VmArray.DATA_OFFSET + tibOffset) * slotSize);
		/* Now invoke the method */
		helper.invokeJavaMethod(methodRef.getSignature());
	...
	public final void visit_ior() {
		...
		switch(v2.getKind()) {
			case Item.REGISTER:
				...
				break;
			case Item.LOCAL:
				os.writeOR(v1.getRegister(), stackFrame.getEbpOffset(v2.getLocal()), FP);
				break;
			case Item.CONSTANT:
				//IMPROVE: implement OR(reg, imm32)
				//os.writeOR(v1.getRegister(), ((IntConstant)v2.getConstant()).getValue());
				v2.load();
				os.writeOR(v1.getRegister(), v2.getRegister());				
				break;
		}
...
//		helper.writePOP(T0);
	...
	public final void visit_ishl() {
		...
		shift.loadToIf(~Item.CONSTANT, ECX);
		value.load();
	...
	public final void visit_isub() {
		...
		switch(v2.getKind()) {
			case Item.REGISTER:
				...
				break;
			case Item.LOCAL:
				os.writeSUB(v1.getRegister(), stackFrame.getEbpOffset(v2.getLocal()), FP);
				break;
			case Item.CONSTANT:
				//IMPROVE: implement SUB(reg, imm32)
				//os.writeSUB(v1.getRegister(), ((IntConstant)v2.getConstant()).getValue());
				v2.load();
				os.writeSUB(v1.getRegister(), v2.getRegister());
				break;
		}
	...
	public final void visit_ixor() {
		...
		switch(v2.getKind()) {
			case Item.REGISTER:
				...
				break;
			case Item.LOCAL:
				os.writeXOR(v1.getRegister(), stackFrame.getEbpOffset(v2.getLocal()), FP);
				break;
			case Item.CONSTANT:
				//IMPROVE: implement XOR(reg, imm32)
				// os.writeXOR(v1.getRegister(), ((IntConstant)v2.getConstant()).getValue());
				v2.load();
				os.writeXOR(v1.getRegister(), v2.getRegister());
				break;
		}
...
//		//log.debug("lconst v=" + NumberUtils.hex(v,16) + ",lsb=" + NumberUtils.hex(lsb, 8) +
...
////			os.writeMOV_Const(Register.EAX, Float.floatToRawIntBits(((Float) value).floatValue()));
...
//			helper.writePUSH(T0);
//		} else {
//			throw new ClassFormatError("ldc with unknown type " + value.getClass().getName());
//		}
////		helper.writePUSH(Register.EAX);
	}
	 ...
	 */
	public final void visit_ldc2(Object value) {
		//TODO: port to orp-style
		long v;
		if (value instanceof Long) {
			visit_lconst(((Long) value).longValue());
//			v = ((Long) value).longValue();
		} else if (value instanceof Double) {
			visit_dconst(((Double) value).doubleValue());
//			v = Double.doubleToRawLongBits(((Double) value).doubleValue());
		} else {
			throw new ClassFormatError("ldc2 with unknown type " + value.getClass().getName());
		}
//		visit_lconst(v);
	}
	...
	public final void visit_lload(int index) {
		vstack.pushItem(LongItem.CreateLocal(index));
//		int ebpOfs = stackFrame.getWideEbpOffset(index);
//		os.writeMOV(INTSIZE, T0, FP, ebpOfs);
...
//		helper.writePUSH64(T0, T1);
	...
	public final void visit_lmul() {
		...
		os.writeMOV(INTSIZE, Register.EAX, Register.ESI); // lo2
		os.writeMUL_EAX(Register.EBX); // lo1*lo2
		os.writeADD(Register.EDX, Register.ECX); // hi2*lo1 + hi1*lo2 + hi(lo1*lo2)
...
//		os.writeJMP(helper.getInstrLabel(defAddress));
	...
	public final void visit_multianewarray(VmConstClass clazz, int dimensions) {
		...
		System.out.println("multianewarray not implemented yet in "
				+ helper.getMethod());
		// Dummy to maintain the stack structure, but cause a NullPointerException
	...
	public final void visit_putfield(VmConstFieldRef fieldRef) {
		...
		if (field.isStatic()) {
			throw new IncompatibleClassChangeError(
					"getfield called on static field " 
					+ fieldRef.getName());
		}
		final VmInstanceField inf = (VmInstanceField) field;
		final int offset = inf.getOffset();
...
//		helper.writePUSH(T0);
	...
	public final void visit_sastore() {
		...
		if (idx.getKind() == Item.CONSTANT) {
			...
			os.writeMOV(WORDSIZE, r, idx.getRegister(), 2, VmArray.DATA_OFFSET * 4, v);			
		}
		val.release();
		idx.release();
		ref.release();
...
//	}
	...
	public final void visit_swap() {
		...
		if (v1IsBool && v2IsBool) {
			// both on the stack: must be popped to be inverted (inverting only on vstack not enough)
			v1.load();
			v2.load();
		}
		vstack.pushItem(v1);
...
//		os.writeJMP(helper.getInstrLabel(defAddress));
	...
	private final void checkBounds(RefItem ref, IntItem index) {
		...
		if (index.getKind() == Item.CONSTANT) {
			...
			index.load();
			os.writeCMP(r, VmArray.LENGTH_OFFSET * slotSize,  index.getRegister());
		} else {
			os.writeCMP(r, VmArray.LENGTH_OFFSET * slotSize,  index.getRegister());			
		}
...
//		os.setObjectRef(ok);	
	...
	/**
	 * Write code to resolve the given constant field referred to
	 * by fieldRef
	 * 
	 * @param fieldRef
	 * @param scratch
	...
	private final void writeInitializeClass(VmConstFieldRef fieldRef, 
		...
		if (!declClass.isInitialized()) {
			...
			helper.writeGetStaticsEntry(new Label(curInstrLabel + "$$ic"), EAX, declClass);
	...
	/**
	 * Write code to resolve the given constant class (if needed) and load 
	 * the resolved class (VmType instance) into EAX.
	 * 
	 * @param classRef
	 * @param scratch
	 */
	...
	private final void instanceOf(Label trueLabel) {
		...
		final Label notInstanceOfLabel = 
			new Label(this.curInstrLabel + "notInstanceOf");
		...
		os.writeMOV(INTSIZE, Register.EDX, Register.EAX);
		/* TIB -> ESI */
		os.writeMOV(INTSIZE, Register.ESI, Register.ECX, ObjectLayout.TIB_SLOT * slotSize);
		/* SuperClassesArray -> ESI */
		os.writeMOV(INTSIZE, Register.ESI, Register.ESI, (VmArray.DATA_OFFSET + TIBLayout.SUPERCLASSES_INDEX) * slotSize);
