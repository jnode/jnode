 ...
 * @author epr
 */
public class DefaultDeviceManager implements DeviceManager, ExtensionPointListener {
	 ...
	 * @param mappersEP
	 */
	public DefaultDeviceManager(ExtensionPoint findersEP, ExtensionPoint mappersEP) {
		if (findersEP == null) {
	 ...
	 * <ul>
	 * <li>Search for a suitable driver for the device. If not found the driver startup is
	 * delayed.
	 * <li>Connect the driver to the device, if a driver is found
	 * <li>Attempt to start the device. If this fails an exception is printed in the log. You can
	 * test if the device was started succesfully, by read the <code>isStarted</code> status.
	 * </ul>
	 * Note that if the device already has a driver connected to it, the first two steps are
	 * ignored.
	 * 
	 * @param device
	 ...
	 */
	public synchronized void register(Device device) throws DeviceAlreadyRegisteredException, DriverException {
		final String devID = device.getId();
		if (devices.containsKey(devID)) {
			throw new DeviceAlreadyRegisteredException(devID);
		}
		// Set a link to me
	...
	/**
	 * Unregister a device. The device will be stopped and removed from the namespace.
	 * 
	 * @param device
	 * @throws DriverException
	 */
	 ...
	 * @throws DeviceAlreadyRegisteredException
	 */
	public synchronized void rename(Device device, String name, boolean autonumber) throws DeviceAlreadyRegisteredException {
		if (!device.getId().startsWith(name)) {
			...
			if (devices.containsKey(newId)) {
				throw new DeviceAlreadyRegisteredException(newId);
			}
			// Remove the old id
			devices.remove(device.getId());
			// Add the new id
	...
	/**
	 * Find a driver from each device that has not yet has a driver connected to it.
	 */
	 ...
	 * 
	 * @param device
	 * @return The first suitable driver for the given device, or a NullDriver if no suitable
	...
	protected Driver findDriver(Device device) {
		...
		BootLog.debug("No driver found for " + device + " delaying device startup");
		return null;
	}
	...
	/**
	 * Configure a finder for a single finder configuration element and add the new finder to the
	 * given list.
	 * 
	 * @param finders
	 * @param element
	...
	/**
	 * Configure a mapper for a single mapper configuration element and add the new mapper to the
	 * given list.
	 * 
	 * @param mappers
	 * @param element
	...
	private void configureMapper(List mappers, ConfigurationElement element) {
		...
		if (className != null) {
			try {
				final Class cls = Thread.currentThread().getContextClassLoader().loadClass(className);
				final DeviceToDriverMapper mapper = (DeviceToDriverMapper) cls.newInstance();
				mappers.add(mapper);
			} catch (ClassNotFoundException ex) {
