	...
	public ParseNode findName(NameString name, int opcode) {
		...
		while (child != null) {
			if (child.isFieldOpcode()) {
				...
				while (field != null) {
					if (field.isNamedOpcode() && field.getName().equals(name) && (opcode != 0 || field.opcode == opcode)) {
						return field;
					}
					field = field.next;
				}
			} else if (child.isNamedOpcode() && child.getName().equals(name) && (opcode != 0 || child.opcode == opcode)) {
				break;
			}
			child = child.next;
		}
