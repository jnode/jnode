...
public class BlockAlignmentSupport implements BlockDeviceAPI {
     ...
     * @param offset
     * @param dst
     * @see org.jnode.driver.block.BlockDeviceAPI#read(long, ByteBuffer)
     * @throws IOException
     */
	...
	case BufferType.CROSSED:
	    ...
	    //
	    if ( t.nB != 0 ) {
		mylog ( "read[BODY] " + t.toBlockString ( t.start + alignment, t.lB ) );
		//
		t.limit ( t.lB );
		parentApi.read ( t.start + alignment, dst );
		t.restore();
	    }
	    //
	    if ( t.lstR != 0 ) {
     ...
     * @param offset
     * @param src
     * @see org.jnode.driver.block.BlockDeviceAPI#write(long, ByteBuffer)
     * @throws IOException
     */
	...
	case BufferType.CROSSED:
	    ...
	    //
	    if ( t.nB != 0 ) {
		mylog ( "write[BODY] " + t.toBlockString ( t.start + alignment, t.lB ) );
		//
		t.limit ( t.lB );
		parentApi.write ( t.start + alignment, src );
		t.restore();
	    }
	    //
	    if ( t.lstR != 0 ) {
	...
	private String toTypeString() {
		...
		if ( nB != 0 )
		    str.append ( "B[" + (start+alignment) + ":" + lB + "] " );
		if ( lstR != 0 )
