...
public class Ext2FileSystem implements FileSystem {
	...
	/*
	 * remarks about the code: -wherever the number (count) of some structure is
	 * known (e.g. number of block groups), I use an array to store them
	 */
	...
	//private int partitionBegin = 31744;
	//private BlockCache cache;
	private int partitionBegin = 0;
	...
	public Ext2FileSystem(Device device) throws FileSystemException {
		...
		//cache = new BlockCache(20,(float)0.75);
		cache = new HashMap();
		...
		try {
			...
			for (int i = 0; i < groupCount; i++) {
				...
				api.read(
					byteIndex + i * GroupDescriptor.GROUPDESCRIPTOR_LENGTH,
					data,
					0,
			...
			/*
			 * //go through each block group //XXX byteIndex = ... //read the
			 * block bitmap //XXX what if it doesn't fit? data = new
			 * byte[(int)superblock.getBlockSize()]; api.read( byteIndex, data, 0,
			 * (int)superblock.getBlockSize()); FSBitmap blockBitmap = new
			 * FSBitmap(data); byteIndex += superblock.getBlockSize();
			 * 
			 * //read the inode bitmap //XXX what if it doesn't fit? data = new
			 * byte[(int)superblock.getBlockSize()]; api.read( byteIndex, data, 0,
			 * (int)superblock.getBlockSize()); FSBitmap inodeBitmap = new
			 * FSBitmap(data); byteIndex += superblock.getBlockSize();
			 * 
			 * //read the inode table //XXX what if it doesn't fit? Inode
			 * inodeTable[] = new Inode[ (int)superblock.getInodesCount() ];
			 */
	...
	public FSEntry getRootEntry() throws IOException {
		...
		if (!closed) {
			return new Ext2Entry(getINode(INode.EXT2_ROOT_INO), "");
		}
		return null;
	}
	...
	public byte[] getBlock(long nr) throws IOException {
      ...
      int blockSize = superblock.getBlockSize();
		...
		if (cache.containsKey(key))
			...
			api.read(nr * blockSize + partitionBegin, data, 0, blockSize);
	 ...
	 */
	public INode getINode(int iNodeNr) throws IOException {
		if (iNodeNr < 1)
		...
		long iNodeTableBlock = groupDescriptors[group].getInodeTable(); //the
		// first
		// block
		// of the
		// inode
