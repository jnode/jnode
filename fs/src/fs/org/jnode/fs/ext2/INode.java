...
/**
 * This class represents an inode. Once they are allocated, inodes are 
 * read and written by the INodeTable (which is accessible through
 * desc.getINodeTable().
 * 
 * @author Andras Nagy
 */
public class INode {
	...
	/**
	 * Called when an inode is flushed from the inode buffer and
	 * its state must be saved to the disk
	 */
	...
	/**
	 * Parse the indirect blocks of level <code>indirectionLevel</code> and return
	 * the address of the <code>offset</code>th block.
	 * 	For example, indirectRead( doubleIndirectBlockNumber, 45, 3) will return
	 * 	the 45th block that is reachable via triple indirection, which is the 
	 *  (12 + getIndirectCount() +getIndirectCount()^2 + 45)th block of the inode 
	 *  (12 direct blocks, getIndirectCount() simple indirect blocks, getIndirectCount()^2
	 * 	double indirect blocks, 45th triple indirect block).  
	 *  
	 * @param indirectionLevel:     0: direct block, 1: indirect block, ...
	 */
	...
	/**
	 * Parse the indirect blocks of level <code>indirectionLevel</code> and register
	 * the address of the <code>offset</code>th block.
	 * Also see indirectRead().
	...
	private final void indirectFree(long dataBlockNr, long offset, int indirectionLevel)
		throws IOException, FileSystemException {
			log.debug("indirectFree(datablockNr="+dataBlockNr+", offset="+offset+", ind="+indirectionLevel+")"); 
	...
	/** 
	 * Return the number of the block in the filesystem that stores 
	 * the ith block of the inode (i is a sequential index from
	 * the beginning of the file)
	 * 
 	 * [Naming convention used: in the code, a <code>...BlockNr</code> always means an absolute 
	 *  block nr (of the filesystem), while a <code>...BlockIndex</code> means an index relative 
	...
	private long getDataBlockNr(long i) throws IOException {
		...
		//shouldn't get here
		throw new IOException(new FileSystemException("Internal FS exception: getDataBlockIndex(i="+i+")"));
	}

	/** Read the ith block of the inode (i is a sequential index from
	...
	/**
	 * A new block has been allocated for the inode, so register it 
	 * (the <code>i</code>th block of the inode is the block at <code>blockNr</code>
	 * 
	 * [Naming convention used: in the code, a <code>...BlockNr</code> always means an absolute 
	 *  block nr (of the filesystem), while a <code>...BlockIndex</code> means an index relative 
	 *  to the beginning of a block]
	 ...
	 */
	private final void registerBlockIndex(long i, long blockNr) throws FileSystemException, IOException{
	...
	protected synchronized void freeDataBlock(long i) throws IOException, FileSystemException {
		...
		//preallocated blocks follow the last allocated block: when the last block is freed,
	...
	/** 
	 * Write the i. data block of the inode (i is a sequential index from
	 * the beginning of the file, and not an absolute block number)
	 * 
	...
	/**
	 * Get the number of blocks allocated so far for the inode.
	 * It is possible that a new block has been allocated, but not yet been written to. In this
	 * case, it is not counted by getSizeInBlocks(), because it returns the size of the file in blocks,
	 * counting only written bytes
	 * @return
	 */
	...
	/** 
	 * Allocate the ith data block of the inode (i is a sequential index
	 * from the beginning of the file, and not an absolute block number)
	 */
	...
	/**
	 * FINDS a free block which will be the indexth block of the inode:
	 * 	-first check the preallocated blocks
	 * 	-then check around the last allocated block
	 *  and ALLOCATES it in the block bitmap at the same time.
	 * 
	 * Block allocation should be contiguous if possible, i.e. the
	 * new block should be the one that follows the last allocated block
	 * (that's why the <code>index</code> parameter is needed).
	 * 
	 * @param index: the block to be found should be around the
	 * (index-1)th block of the inode (which is already allocated, unless
	 * index==0)
	 */
	private long findFreeBlock(long index) throws IOException, FileSystemException{
		...
		//(threshold=1 means: find is successul if at least one free block is found)
		...
		for(int i=0; i<getExt2FileSystem().getGroupCount(); i++) {
			...
			long threshold = (long)Math.ceil( 	getExt2FileSystem().getSuperblock().getBlocksPerGroup() * 
	...
	/**
	 * Return the size in ext2-blocks (getBlocks() returns the size in
	 * 512-byte blocks, but an ext2 block can be of different size).
	 * @return
	 */
