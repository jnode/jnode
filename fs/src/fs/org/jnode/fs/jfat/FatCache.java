...
public class FatCache {
	...
	Logger.getLogger ( FatCache.class );
	...
	this.fat = fat;
	this.api = fat.getApi();
	this.fatsize = fat.getBootSector().getSectorsPerFat() *
	    fat.getBootSector().getBytesPerSector();
    ...
    private CacheElement put ( long address )
	throws IOException {
	/**
	...
	/**
	 * stack "must" contains at least one entry
	 *   the placeholder ... so let it throw an exception
	 *     if this is false
	 */
	...
	//   this will update the LRU order: the LinkedHashMap will make it the newest
	//
	// the cache element cannot be null so we can avoid to call containsKey();
	if ( c != null )
	    hit++;
    ...
    private long getUInt32 ( long offset )
	throws IOException {
    ...
    private void setInt32 ( long offset, int value )
	throws IOException {
	long addr = (long)(offset / elementSize);
	int  ofs  = (int) (offset % elementSize);
    ...
    public void flush ( long address )
	throws IOException {
	CacheElement c = map.get ( address );
	if ( c != null )
	    c.flush();
	...
	out.print ( map );
	out.println ( "size=" + getCacheSize() + " used=" + usedEntries() +
		      " free=" + freeEntries() );
	...
	protected boolean removeEldestEntry ( Map.Entry<CacheKey,CacheElement> eldest ) {
	    ...
	    /**
	     * before going to discard the eldest
	     *    push it back on the stacked object pool
	     */
	    if ( remove ) 
    ...
    /**
     * Here we need to "wrap" a long because Java Long
     * wrapper is an "immutable" type
     */
	...
	/** 
	 * CacheKey element is allocated and its
	 *   reference is stored here to avoid to allocate
	 *    new CacheKey objects at runtime
	 *
	 * In this way .. just one global key will be enough
	 * to access CacheElements
	 */
	...
	/**
	 * some more work is needed in read and write
	 * to handle the multiple fat availability
	 * we have to correcly handle the exception to be
	 * sure that if we have at least a correct fat
	 * we get it - gvt
	 */
	private void read ( long address )
	    throws IOException {
	    if ( !isFree() )
