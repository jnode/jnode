...
public class FatDirectory extends FatEntry implements FSDirectory {
    ...
    private static final Logger log =
        Logger.getLogger ( FatDirectory.class );
    ...
    /*
     * this is actually a FatDirEntry factory
     * and not a standard read method ... but how would you call it?
     */
    private FatDirEntry getFatDirEntry ( int index )
	throws IOException {
     ...
     */
    public void setFatDirEntry ( FatDirEntry entry )
	throws IOException {
	getChain().write ( entry.getIndex() * entry.length(), entry.getByteBuffer() );
    }


    public FatDirEntry[] getFatFreeEntries ( int n )
	throws IOException {
	int i = 0;
	int index = 0;
	...
	protected EntriesFactory() {
 	...
 	protected boolean hasNextEntry() {
	    ...
	    /*
	     * here recursion is in action for the entries factory
	     * it creates directory nodes and file leafs
	     */
	    if ( ((FatShortDirEntry)e).isDirectory() )
		this.entry = new FatDirectory
		    ( getFatFileSystem(), FatDirectory.this, v );
	    else 
		this.entry = new FatFile
		    ( getFatFileSystem(), FatDirectory.this, v );
