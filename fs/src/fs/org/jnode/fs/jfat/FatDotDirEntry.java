...
public class FatDotDirEntry extends FatShortDirEntry {
    private static final Logger log =
        Logger.getLogger ( FatDotDirEntry.class );
    ...
    public FatDotDirEntry ( FatFileSystem fs, boolean dotDot,
			    FatShortDirEntry parent, int startCluster )
	throws IOException {
	super ( fs );
    ...
    private void init ( FatShortDirEntry parent, int startCluster )
	throws IOException {
	setNameCase ( new FatCase() );
	setAttr ( new FatAttr() ); setDirectory();
	setCreated ( parent.getCreated() );
	clearName(); lName[0] = dot;
