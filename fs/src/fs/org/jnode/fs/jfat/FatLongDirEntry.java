...
public class FatLongDirEntry extends FatDirEntry {
    ...
    public FatLongDirEntry ( FatFileSystem fs, FatMarshal entry, int index )
	throws IOException {
    ...
    public FatLongDirEntry ( FatFileSystem fs, String component,
			     byte ordinal, byte chksum, boolean last,
			     int index )
	throws IOException {
	...
	if ( ordinal < LONG_ENTRY_MIN || ordinal > LONG_ENTRY_MAX )
		...
		( "ordinal is invalid: " + NumberUtils.hex ( ordinal, 2 ) );
	...
	/*
	 * if the name is Unicode NUL (0x0000) terminated "l" will
	 * end with the Unicode string length ( < 13 )
	 * otherwise the length will be exactly 13
	 * and will not be terminated or padded at all
	 */
	for ( l = 0; l < m; l++ )
	    if ( ( lName[2*l]   == (byte)0x00 ) &&
		 ( lName[2*l+1] == (byte)0x00 ) )
		break;

	/*
	 * check to see if the name is correctly 0xFFFF padded
	 * (page 28) - note that the name is not padded if
	 * it exactly fits the 13 Unicode char position
	 * and it is only NUL terminated if has a length of 12
	 */
	for ( i = l+1; i < m; i++ ) {
	    if ( ( lName[2*i]   != (byte)0xFF ) ||
	...
	/*
	 * if the entry is not padded correctly is marked "damaged"
	 * and will considered an orphan entry
	 * the thread will not reach this point ... game is over ...
	 */
	...
	if ( l > lName.length )
	    throw new IllegalArgumentException
		( "component length exceed limit: " + l );
	
	if ( l == 0 )
	    throw new IllegalArgumentException
		( "component has zero length" );
	
	if ( ( l % 2 ) == 1 )
	    throw new IllegalArgumentException
		( "component has an odd byte length: " + l );
	...
	/*
	 * NUL terminate the component and PAD with 0xFFFF
	 * if needed
	 */
	if ( l < m ) {
	    lName[2*l]   = (byte)0x00;
