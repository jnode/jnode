...
public class FatRecord {
    private static final Logger log =
        Logger.getLogger ( FatRecord.class );
    ...
    public FatRecord ( FatDirectory parent, FatName name )
	throws IOException {
	init();
	...
	if ( name.isMangled() ) {
	    ...
	    for ( int i = (n-2); i >= 0; i-- ) {
		l = new FatLongDirEntry
		    ( fs, name.getComponent(i), (byte)(i+1), s.getChkSum(),
		      false, free[n-1-i].getIndex() );
    ...
    private void addSetFatDirEntry ( FatDirectory parent, FatLongDirEntry entry )
	throws IOException {
	add ( entry );
	...
	int i;
	StringBuilder lname =
	     new StringBuilder ( longEntries.size() * FatLongDirEntry.NAMELENGTH );
