...
public class FatUtils {
	...
	if (  dosDate < 0 || dosDate > 0xFFFF )
		...
		( "dosDate is invalid: " + dosDate );

	if (  dosTime < 0 || dosTime > 0xFFFF )
	    throw new IllegalArgumentException
		( "dosTime is invalid: " + dosTime );
	
	if (  dosTenth < 0 || dosTenth > 199 )
	    throw new IllegalArgumentException
		( "dosTime is invalid: " + dosTenth );
	...
	if ( javaDateTime < dosMinDateTime ||
	     javaDateTime > getMaxDateTime() )
	...
	return 100 * ( cal.get ( Calendar.SECOND ) % 2        ) +
