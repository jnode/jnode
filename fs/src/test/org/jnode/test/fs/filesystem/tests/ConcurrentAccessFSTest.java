...
*/	
	protected void createReaders(Monitor monitor, FSFile file)
	{
		for(int i = 0 ; i < NB_READERS ; i++)
			monitor.addWorker(new Reader(monitor, file, i * 2, NB_READERS * 2, MIN_SLEEP, MAX_SLEEP));		
	}

	protected void createWriters(Monitor monitor, FSFile file)
	{
		for(int i = 0 ; i < NB_WRITERS ; i++)
			monitor.addWorker(new Writer(monitor, file, i * 2, NB_WRITERS * 2, MIN_SLEEP, MAX_SLEEP));		
	}
...
{
	public Worker(Monitor monitor, FSFile file, int start, int step, int minSleep, int maxSleep)
	{
		this.file = file;
		this.step = step;
		this.start = start;
...
class Reader extends Worker
{
	 ...
	 * @param file
	 */
	public Reader(Monitor monitor, FSFile file, int start, int step, int minSleep, int maxSleep) {
...
class Writer extends Worker
{
	 ...
	 * @param file
	 */
	public Writer(Monitor monitor, FSFile file, int start, int step, int minSleep, int maxSleep) {
		super(monitor, file, start, step, minSleep, maxSleep);
	}
...
class Monitor
{
	...
	public void waitAll() throws Throwable
	{
		...
		//System.out.println("Monitor is waiting for "+workers.size()+" workers");
	...
	public void notifyError(Worker worker, Throwable throwable)
	{
		...
		this.throwable = new Error(worker.getClass().getName()+" failed !", throwable);
	}
