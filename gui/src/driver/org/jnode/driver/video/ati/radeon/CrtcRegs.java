...
final class CrtcRegs implements RadeonConstants {
	...
	public final void restoreToVGA(RadeonVgaIO io) {
		if (crtcIndex == 0) {
			...
			io.setRegP32(CRTC_EXT_CNTL, crtc_ext_cntl, CRTC_VSYNC_DIS
					| CRTC_HSYNC_DIS | CRTC_DISPLAY_DIS);
			...
			io.setRegP32(CRTC2_GEN_CNTL, crtc_gen_cntl, CRTC2_VSYNC_DIS
					| CRTC2_HSYNC_DIS | CRTC2_DISP_DIS | CRTC2_CRT2_ON);
	 ...
	 * @param io
	 */
	final void calcForConfiguration(RadeonConfiguration config,
			PLLInfo pllInfo, RadeonVgaIO io, FBInfo fbinfo) {
		...
		final int hsync_wid = Math.min(0x3f, Math.max(1,
				(hSyncEnd - hSyncStart) / 8));
		final int vsync_wid = Math
				.min(0x1f, Math.max(1, vSyncEnd - vSyncStart));
		...
		this.crtc_h_total_disp = ((hTotal / 8 - 1) & 0x3ff)
				| ((xres / 8 - 1) << 16);
		this.crtc_h_sync_strt_wid = ((hsync_start & 0x1fff) | (hsync_wid << 16) | (h_sync_pol << 23));

		this.crtc_v_total_disp = ((vTotal - 1) & 0xffff) | ((yres - 1) << 16);

		this.crtc_v_sync_strt_wid = (((vSyncStart - 1) & 0xfff)
				| (vsync_wid << 16) | (v_sync_pol << 23));
	...
	public String toString() {
		return "crtc_h_total_disp:" + crtc_h_total_disp
				+ ", crtc_h_sync_strt_wid:" + crtc_h_sync_strt_wid
				+ ", crtc_v_total_disp:" + crtc_v_total_disp
				+ ", crtc_v_sync_strt_wid:" + crtc_v_sync_strt_wid
				+ ", crtc_pitch:0x" + NumberUtils.hex(crtc_pitch)
				+ ", crtc_gen_cntl:0x" + NumberUtils.hex(crtc_gen_cntl)
				+ ", crtc_ext_cntl:0x" + NumberUtils.hex(crtc_ext_cntl)
				+ ", dac_cntl:0x" + NumberUtils.hex(dac_cntl)
