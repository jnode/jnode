...
//		io.setReg32(FP_VERT_STRETCH, fp_vert_stretch);
	...
	final void calcForConfiguration(RadeonConfiguration config,
			PLLInfo pllInfo, RadeonVgaIO io, FBInfo fbinfo, CrtcRegs crtc) {
		...
		if ((hRatio == 1.0f) || true) {
			...
			fp_horz_stretch = (((int) ((hRatio * HORZ_STRETCH_RATIO_MAX + 0.5)) & HORZ_STRETCH_RATIO_MASK))
					| (fp_horz_stretch & (HORZ_PANEL_SIZE
							| HORZ_FP_LOOP_STRETCH | HORZ_AUTO_RATIO_INC));
			fp_horz_stretch |= (HORZ_STRETCH_BLEND | HORZ_STRETCH_ENABLE);
		...
		if ((vRatio == 1.0f) || true) {
			...
			fp_vert_stretch = (((((int) (vRatio * VERT_STRETCH_RATIO_MAX + 0.5)) & VERT_STRETCH_RATIO_MASK)) | (fp_vert_stretch & (VERT_PANEL_SIZE | VERT_STRETCH_RESERVED)));
			fp_vert_stretch |= (VERT_STRETCH_ENABLE | VERT_STRETCH_BLEND);
		...
		fp_gen_cntl &= ~(FP_SEL_CRTC2 | FP_RMX_HVSYNC_CONTROL_EN
				| FP_DFP_SYNC_SEL | FP_CRT_SYNC_SEL | /*FP_CRTC_LOCK_8DOT
				| FP_USE_SHADOW_EN |*/ FP_CRTC_USE_SHADOW_VEND | FP_CRT_SYNC_ALT);
		...
		if (fbinfo.getPrimaryMonitorType() == MonitorType.LCD) {
			...
			/* DFP */
			fp_gen_cntl |= (FP_FPON | FP_TMDS_EN);
			tmds_transmitter_cntl = (TMDS_RAN_PAT_RST | ICHCSEL | TMDS_PLL_EN)
					& ~(TMDS_PLLRST);
		}
	...
	public String toString() {
		return "fp_crtc_h_total_disp:" + fp_crtc_h_total_disp
				+ ", fp_crtc_v_total_disp:" + fp_crtc_v_total_disp
				+ ", fp_gen_cntl:0x" + NumberUtils.hex(fp_gen_cntl)
				+ ", fp_h_sync_strt_wid:" + fp_h_sync_strt_wid
				+ ", fp_horz_stretch:" + fp_horz_stretch
				+ ", fp_v_sync_strt_wid:" + fp_v_sync_strt_wid
				...
				+ ", tmds_transmitter_cntl:0x"
				+ NumberUtils.hex(tmds_transmitter_cntl);
	}
}