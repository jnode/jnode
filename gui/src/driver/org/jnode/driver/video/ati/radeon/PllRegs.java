...
//		io.setPLLP(PPLL_CNTL, 0, ~PPLL_RESET);	
		...
		io.setPLLP(PPLL_CNTL, PPLL_RESET | PPLL_ATOMIC_UPDATE_EN
				| PPLL_VGA_ATOMIC_UPDATE_EN, ~(PPLL_RESET
		 ...
		 */
		for (int i = 0; (i < 10000)
				&& ((io.getPLL(PPLL_REF_DIV) & PPLL_ATOMIC_UPDATE_R) != 0); i++) {
			// Wait
		}
		...
		io.setPLLP(PPLL_CNTL, 0, ~(PPLL_RESET | PPLL_SLEEP
				| PPLL_ATOMIC_UPDATE_EN | PPLL_VGA_ATOMIC_UPDATE_EN));
...
//					+ ", bitvalue=" + bitvalue);
	...
	public String toString() {
		return "ppll_ref_div:" + ppll_ref_div + ", ppll_div_3:0x"
				+ NumberUtils.hex(ppll_div_3) + ", htotal_cntl:0x"
				+ NumberUtils.hex(htotal_cntl);
	}
