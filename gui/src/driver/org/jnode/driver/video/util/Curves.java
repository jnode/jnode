...
public class Curves {
	 ...
	 */
	public static int calculateCubicCurve(double x0, double y0, double x1, double y1, double x2, double y2, double x3, double y3, double points[]) {
	 ...
	 */
	public static int calculateCubicCurveOpt2(double x0, double y0, double x1, double y1, double x2, double y2, double x3, double y3, double[] points) {
	 ...
	 * @param points The resulting points
	 */
	public static int calculateQuadCurve(double x0, double y0, double x1, double y1, double x2, double y2, double points[]) {
	 ...
	 * @return int number of points in points array
	 */
	public static int calculateQuadCurveOpt2(double x0, double y0, double x1, double y1, double x2, double y2, double points[]) {
	 ...
	 * also I haven't been able to find a good estimation how many subdiv steps we need
	 * 
	 * @param x0
	 * @param y0
	 * @param x1
	 ...
	 * @param points The resulting points
	 *
	public static GeneralPath calculateQuadCurveOpt1(double x0, double y0, double x1, double y1, double x2, double y2) {
		
		GeneralPath result = new GeneralPath();
		result.moveTo((float) x0, (float) y0);
		calculateQuadCurveRecursive(x0, y0, x1, y1, x2, y2, result);
		
		return result;
	}
	
	private static void calculateQuadCurveRecursive(double x0, double y0, double x1, double y1, double x2, double y2, GeneralPath result) {
	
		// try to estimate if we have to do a further subdivision
		double dx = x2-x0;
        double dy = y2-y0;

        /*
        double dist = Math.abs(((x1 - x2) * dy - (y1 - y2) * dx));
        // if the distance from point 1 to the line (0 to 2) is less then one pixel
        // TODO: dist is not the distance as expected!
		if (dist < 2.0) {
			result.addDoubles(x1, y1);
			result.addDoubles(x2, y2);
			return;
		}
        
        // this is an alternativ to the first aproximation, don't know which one is faster or better yet :/
		if ( ((dx*dx) + (dy*dy)) < 4 ) {
			result.lineTo((float) x2, (float) y2);
			return;
		}

		// calculate mid-points
		// this is only fast if l1a compiler uses fscale instead of fdiv!!
		double x01 = (x0 + x1) / 2;
		double y01 = (y0 + y1) / 2;
		double x12 = (x1 + x2) / 2;
		double y12 = (y1 + y2) / 2;
