 ...
 *
 */
public class VESACore extends AbstractSurface implements VESAConstants, PCI_IDs, HardwareCursorAPI {
	 ...
	 * @param device
	 */
	public VESACore(VESADriver driver, VbeInfoBlock vbeInfoBlock, ModeInfoBlock modeInfoBlock, PCIDevice device) throws ResourceNotFreeException, DriverException {
		...
		try {
			...
			int multiplier = 1;
			if(bitsPerPixel == 16) multiplier = 2;
			else if(bitsPerPixel == 24) multiplier = 3;
			else if(bitsPerPixel == 32) multiplier = 4;
			
			this.bytesPerLine = modeInfoBlock.getXResolution() * multiplier;
			this.videoRamSize = bytesPerLine * modeInfoBlock.getYResolution();
			
			this.videoRam = rm.claimMemoryResource(device, address, videoRamSize, ResourceManager.MEMMODE_NORMAL);
			
			Unsafe.debug("\nVESACore : maxWidth="+maxWidth+" maxHeight="+maxHeight+
					" bitsPerPixel="+bitsPerPixel+" bytesPerLine="+bytesPerLine+
					" videoRam="+NumberUtils.hex(modeInfoBlock.getRamBase())+" videRamSize="+videoRamSize);
			
			BitmapGraphics graphics;
			switch (bitsPerPixel) {
				case 8 :
					{
						graphics = BitmapGraphics.create8bppInstance(videoRam, width, height, bytesPerLine, 0);
				...
				case 16 :
					{
						graphics = BitmapGraphics.create16bppInstance(videoRam, width, height, bytesPerLine, 0);
				...
				case 24 :
					{
						graphics = BitmapGraphics.create24bppInstance(videoRam, width, height, bytesPerLine, 0);
				...
				case 32 :
					{
						graphics = BitmapGraphics.create32bppInstance(videoRam, width, height, bytesPerLine, 0);
...
//		}
	...
	public final void setMode(int width, int height, ColorModel model) {
		...
		switch (bitsPerPixel) {
			case 8 :
				{
					graphics = BitmapGraphics.create8bppInstance(videoRam, width, height, bytesPerLine, offset);
					bitmapGraphics.setBitmapGraphics(graphics);
				}
				break;
			case 16 :
				{
					graphics = BitmapGraphics.create16bppInstance(videoRam, width, height, bytesPerLine, offset);
					bitmapGraphics.setBitmapGraphics(graphics);
				}
				break;
			case 24 :
				{
					graphics = BitmapGraphics.create24bppInstance(videoRam, width, height, bytesPerLine, offset);
					bitmapGraphics.setBitmapGraphics(graphics);
				}
				break;
			case 32 :
				{
					graphics = BitmapGraphics.create32bppInstance(videoRam, width, height, bytesPerLine, offset, model.getTransparency());
					bitmapGraphics.setBitmapGraphics(graphics);
				}
				break;
		}
	...
	public FrameBufferConfiguration[] getConfigs() {
		...
		Unsafe.debug("\nbitsPerPixel="+bitsPerPixel+" redMask="+NumberUtils.hex(redMask)+" greenMask="+NumberUtils.hex(greenMask)+
				" blueMask="+NumberUtils.hex(blueMask)+" alphaMask="+NumberUtils.hex(alphaMask));
	...
	public final void fillRect(int x, int y, int width, int height, int color, int mode) {
		Unsafe.debug("\nfillRect x="+x+" y="+y+" width="+width+" height="+height+" color="+color+" mode="+mode);
	...
	protected final void drawPixel(int x, int y, int color, int mode) {
		Unsafe.debug("\ndrawPixel : x="+x+" y="+y+" color="+color+" mode="+mode);		
	...
	public final void drawLine(int x1, int y1, int x2, int y2, int c, int mode) {
		Unsafe.debug("\ndrawLine x1="+x1+" y1="+y1+" x2="+x2+" y2="+y2+" c="+c+" mode="+mode);		
		if (x1 == x2) {
			// Vertical line
