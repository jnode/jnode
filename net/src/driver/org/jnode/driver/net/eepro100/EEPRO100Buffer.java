...
public class EEPRO100Buffer implements EEPRO100Constants {
    ...
    public final int rx(EEPRO100Driver driver) throws NetworkException {
        ...
        int count;
        while (rxRing[entry] != null && ((count = rxRing[entry].getCount()) & PacketReceived) == PacketReceived) {
            int pkt_len = count & 0x07ff;

            if (--rxWorkLimit < 0) break;
            status = rxRing[entry].getStatus();
            log.debug(" rx() status " + Integer.toHexString(status) + " len " + pkt_len);

            if ((status & (RxErrTooBig | RxOK | 0x0f90)) != RxOK) {
                if ((status & RxErrTooBig) != 0)
                    log.debug(": Ethernet frame overran the Rx buffer, status " + Integer.toHexString(status));
                else if (!((status & RxOK) != 0)) {
                    /* There was a fatal error. This *should* be impossible. */
                    rxErrors++;
                    log.debug("Anomalous event in rx(), status " + Integer.toHexString(status));
                }
            } else {
                /*
    ...
    final void setRxMode() {
        ...
        curTx++;
        txRing[entry].setLink(txRing[(entry + 1) & TX_RING_SIZE - 1].getBufferAddress());
    ...
    final void resume() {
        ...
        int rxRingAddress = (rxRing[getCurRx() & RX_RING_SIZE - 1]).getBufferAddress();
        ...
        int txRingAddress = (txRing[getDirtyTx() & TX_RING_SIZE - 1]).getBufferAddress();
