...
public class IPv4NetworkLayer implements NetworkLayer, IPv4Constants, IPv4Service {
	...
	throws SocketException {
		...
		if (skbuf.getSize() < hdr.getDataLength()) {
			stat.badlen.inc();
			//log.debug("Ignored IP packet, mismatch between datalength and buffersize");
			return;
		}
		
		// Get my IP address
		final IPv4ProtocolAddressInfo myAddrInfo = (IPv4ProtocolAddressInfo)deviceAPI.getProtocolAddressInfo(getProtocolID());
		if (myAddrInfo == null) {
			stat.nodevaddr.inc();
		}
	...
	throws SocketException {
		...
		try {
			...
			log.debug("Found unknown IP src=" + hdr.getSource() + ", dst=" + hdr.getDestination() + ", prot=0x" + NumberUtils.hex(hdr.getProtocol(), 2));
		}
	}
		...
		if (flist == null) {
			...
			if (flist.isAlive()) {
				...
				if (flist.isComplete()) {
					...
					//log.debug("Added fragment(" + hdr.getFragmentOffset() + ") to list " + key);
				}
			} else {
	...
	/**
	 * @see org.jnode.net.ipv4.IPv4Service#transmit(org.jnode.net.ipv4.IPv4Header, org.jnode.net.SocketBuffer)
	 */
