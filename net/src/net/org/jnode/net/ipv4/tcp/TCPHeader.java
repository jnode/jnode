...
public class TCPHeader implements TransportLayerHeader, TCPConstants {
	 ...
	 * @param urgentPointer
	 */
	public TCPHeader(int srcPort, int dstPort, int tcpLength, int seqNr, int ackNr, int windowSize, int urgentPointer) {
	...
	public TCPHeader(SocketBuffer skbuf) {
		...
		//Syslog.debug("optionHdrLength 0x" + NumberUtils.hex(optionHdrLength, 4));
		...
		if (checksum == 0) {
			...
			/*
			 * final int ccs1 = IPv4Utils.calcChecksum(phdr, 0, 12); final int ccs2 =
			 * IPv4Utils.calcChecksum(skbuf, 0, udpLength, ccs1);
			 */
			final int ccs2 = IPv4Utils.calcChecksum(phdr, 0, headerLength + tcpLength + 12);
			this.checksumOk = (ccs2 == 0);
			if (!checksumOk) {
				log.debug(
					"Found invalid TCP checksum 0x"
						+ NumberUtils.hex(ccs2, 4)
						+ ", tcpLength 0x"
						+ NumberUtils.hex(tcpLength, 4)
						+ ", ipDataLength 0x"
						+ NumberUtils.hex(ipHdr.getDataLength(), 4)
						+ ", tcpHdrLen 0x"
	...
	/**
	 * Finalize the header in the given buffer. This method is called when all layers have set
	 * their header data and can be used e.g. to update checksum values.
	 * 
	 * @param skbuf
	 *            The buffer
	 * @param offset
	 *            The offset to the first byte (in the buffer) of this header (since low layer
	 *            headers are already prefixed)
	 */
	public void finalizeHeader(SocketBuffer skbuf, int offset) {
		skbuf.set16(offset + 16, 0);
		final int ccs = calcChecksum(skbuf, offset);
		//Syslog.debug("ccs=0x" + NumberUtils.hex(ccs, 4));
	...
	private int calcChecksum(SocketBuffer skbuf, int offset) {
		...
		/*
		 * for (int i = 0; i
		 * < csLength; i++) { System.out.print(NumberUtils.hex(phdr.get(i), 2) + " "); }
		 */
